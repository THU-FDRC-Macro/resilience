...
class AssetMarket(Market):
    ...
    def clear_the_market(self):
        ...
        for atype, v in self.quantities_sold.items():
            if (
                self.model.parameters.PREDEFAULT_FIRESALE_CONTAGION
                or self.model.parameters.POSTDEFAULT_FIRESALE_CONTAGION
            ):
    ...
    def compute_price_impact(self, assetType, qty_sold):
        ...
        new_price = max(
            0, current_price + linear_price_impact(fraction_sold, price_impact)
        )
